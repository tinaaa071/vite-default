...
var _chunkKTXRBLM5cjs = require('./chunk-KTXRBLM5.cjs');
...
function nuxt_default(options = {}, nuxt) {
  ...
  nuxtApp.hook("webpack:config", (configs) => {
    configs.forEach((config) => {
      ...
      config.plugins.unshift(_chunkKTXRBLM5cjs.src_default.webpack(options));
    });
  });
  nuxtApp.hook("vite:extend", async (vite) => {
    vite.config.plugins = vite.config.plugins || [];
    vite.config.plugins.push(_chunkKTXRBLM5cjs.src_default.vite(options));
  });
}
